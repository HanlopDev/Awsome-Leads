...
JWT_SECRET = "gsjdbsgtxvsfagwbwgtdvxcjstwdevcvbc"
...
async def get_ser_by_email(email: str, db: _orm.Session):
...
async def authenticate_user(email: str, password: str, db: _orm.Session):
    user = await get_ser_by_email(db=db, email=email)
...
async def create_token(user: _models.User):
    ...
    token = _jwt.encode(
        user_obj.dict(), JWT_SECRET
    )

    return dict(access_token= token, token_type= "bearer")

async def get_current_user(db: _orm.Session = _fastapi.Depends(get_db), token: str = _fastapi.Depends(oath2schema)):
    try:
